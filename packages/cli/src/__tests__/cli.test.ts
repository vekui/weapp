...
describe("vekui CLI", () => {
  ...
  it("doctor catches dangling local bare css imports", async () => {
    const cwd = await fixture()
    const errors: string[] = []

    await runCli(["init", "--cwd", cwd, "--yes"], { stdout: () => undefined })
    await writeFile(path.join(cwd, "src/styles/bad.css"), '@import "./missing.css";\n')
  ...
  it("doctor catches dangling local unquoted css url imports", async () => {
    const cwd = await fixture()
    const errors: string[] = []
